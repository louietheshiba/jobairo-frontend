  ...
  (
    ...
    return (
      <div className="relative inline-block" ref={dropdownRef}>
        ...
        >
          <p className="mb-0.5">
            {isMulti ? children : selectedValues[0] || children}
          </p>
          ...
          <div className="absolute left-0 z-[99999] mt-2 max-h-[200px] w-auto min-w-full overflow-y-auto rounded-md bg-white shadow-lg ring-1 ring-gray-200 dark:bg-dark-30 dark:ring-dark-15">
            <ul className="py-1">
              {options.map((option) => (
                <li
                  key={option.value}
                  className="flex cursor-pointer items-center whitespace-nowrap px-3 py-1 font-poppins text-sm hover:bg-gray-100 dark:text-white dark:hover:bg-dark-20"
                  onClick={() => handleSelect(option.value)}
                >
                  <input
                    type={isMulti ? 'checkbox' : 'radio'}
                    name={isMulti ? undefined : 'dropdown-single'}
